...
public class NodeSelector {
    ...
    protected final List<Node> allowedNodes;

    public NodeSelector(Node rootNode, List<Node> allowedNodes) {
        this.rootNode = rootNode;
        this.allowedNodes = allowedNodes;
    }

    public NodeSelector(Node rootNode) {
        this(rootNode, Collections.emptyList());
    }
    ...
    protected List<Node> select(Node rootNode, List<CssSelector> selectors) {
        ...
        for (Iterator<Node> iter = nodes.iterator() ; iter.hasNext() ; ){
            ...
            Node node = iter.next();
            if (!node.equals(allowedNodes.get(0))){
                iter.remove();
            }
